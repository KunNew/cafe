  ...
  <v-container fluid>
    <v-card class="mb-3">
      ...
      <template v-else>
        ...
        <v-card-subtitle>List all products.</v-card-subtitle>
      </template>
    </v-card>
    ...
    >
      <template #[`top`]>
        <v-toolbar flat>
          <v-btn class="ma-1" color="primary" to="product/create"
        ...
        <v-tooltip top>
          <template #activator="{ on, attrs }">
            <v-btn
              ...
              small
              :to="
                localePath({ name: 'product-show-id', params: { id: item.id } })
              "
              v-on="on"
...
export default {
  ...
  methods: {
    ...
    onConfirmDelete() {
      this.deleting = true
      axiosApiInstance
        .delete(`product/${this.deleteItem}`)
        .then((response) => {
          this.fetchData()
          this.$store.dispatch('pushNotification', {
            message: response.data.message,
