...
<template>
  <v-container fluid>
      ...
      <v-data-table
          ...
          <template #[`top`]>
              <v-toolbar flat>
                  <v-btn class="ma-1" color="primary" to="user/create"><v-icon left>mdi-plus</v-icon>Add new</v-btn>
                  <v-spacer></v-spacer>
          ...
          <template #[`item.actions`]="{ item }">
              <v-tooltip top>
                  <template #activator="{ on, attrs }">
                      <v-btn color="primary" v-bind="attrs" icon small :to="{ name: 'user-show-id', params: { id: item.id } }" v-on="on"><v-icon>mdi-eye</v-icon></v-btn>
                  </template>
                  <span>Detail</span>
              </v-tooltip>
              <v-tooltip top>
                  <template #activator="{ on, attrs }">
                      <v-btn color="warning" icon small :to="{ name: 'user-id', params: { id: item.id } }" v-bind="attrs" v-on="on"><v-icon>mdi-pencil-box-outline</v-icon></v-btn>
                  </template>
                  <span>Edit</span>
              </v-tooltip>
              <v-tooltip top>
