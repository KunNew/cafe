<template>
  <v-container fluid>
    ...
    >
      <template #[`top`]>
        <v-toolbar flat>
          <v-btn class="ma-1" color="primary" to="category/create"
        ...
        <v-tooltip top>
          <template #activator="{ on, attrs }">
            <v-btn
              ...
              small
              :to="
                localePath({ name: 'category-show-id', params: { id: item.id } })
              "
              v-on="on"
        ...
        <v-tooltip top>
          <template #activator="{ on, attrs }">
            <v-btn
              ...
              icon
              small
              :to="localePath({ name: 'category-id', params: { id: item.id } })"
              v-bind="attrs"
              v-on="on"
...
export default {
  ...
  methods: {
    ...
    onConfirmDelete() {
      this.deleting = true
      axiosApiInstance
        .delete(`category/${this.deleteItem}`)
        .then((response) => {
          this.fetchData()
          this.$store.dispatch('pushNotification', {
            message: response.data.message,
